...
void my_printf(const char *format,...)
{
    ...
    vprintf(format,args); //±ØÐëÓÃvprintf
    va_end(args);
	fflush(stdout);
}
/*******************************************************************************************/
...
void CALLBACK TimerProc(HWND hWnd, UINT nMsg, UINT nIDEvent, DWORD dwTime) 
{
	...
	if (m_OperationCode)  
	{
		...
		my_printf( " Duration: %.2i:%.2i:%.2i", elapsedTime.GetHours(),elapsedTime.GetMinutes(),elapsedTime.GetSeconds());
		...
		if (Context.ErrorCode!=STDFUPRT_NOERROR)
		{
			...
			if (Context.ErrorCode==STDFUPRT_UNSUPPORTEDFEATURE)
				...
				my_printf("This feature is not supported by this firmware.");
			else
						...
						if (m_BufferedImage==0) // This was a standard Upload
						{
							...
							if (dwRet==STDFUFILES_NOERROR)
							{
								...
								if (dwRet==STDFUFILES_NOERROR)
								{
									...
									my_printf("Unable to append image to DFU file...");
								STDFUFILES_CloseDFUFile(hFile);
							}
							else
								my_printf("Unable to create a new DFU file...");
						}
		...
		while(arg_index < argc)
		{
			...
			else if (strcmp(argv[arg_index],"-u")==0)
			{
			   ...
			   if(!FileExist((LPCTSTR)m_UpFileName))
			   {
                      if ( m_UpFileName != "")
					  {
				          my_printf( "file %s does not exist .. Creating file\n", m_UpFileName);
			...
			else if (strcmp(argv[arg_index],"-d")==0)
			{
			  ...
			  if(!FileExist((LPCTSTR)m_DownFileName))
			   {
                    my_printf( "file does not exist %s \n", m_DownFileName);
